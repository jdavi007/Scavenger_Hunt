...
# Event loader - updated and working
...
# Event Page - updated and working
# Precondition: Student login required
...
def event(eventID):
    ...
    if request.method == 'POST':
        ...
        for task in tasks:
            ...
            if response:
                conn.execute('UPDATE tasks SET hasResponse = ?, response = ? WHERE task_id = ?', (1, response, task_id))
        ...
        if action == "completeTask":
            ...
            if all(task['hasResponse'] for task in tasks):
    ...
    return render_template('event.html', event=event, user_role=userRole)
# end event()
...
# Completed Event Page - updated and working
...
# Event Creation Page - needs work
# Precondition: Staff login required
        ...
        if not title:
            ...
            try:
                ...
                for description, upload_type in zip(taskDescriptions, uploadTypes):
                    cursor.execute('INSERT INTO tasks (event_id, description, uploadType, hasResponse) VALUES (?, ?, ?, ?)', (event_id, description, upload_type, 0))
...
# Edit Event Page - needs work, might need to look at database to fix
# Precondition: Staff login required
    ...
    if request.method == 'POST':
        ...
        if not title:
            ...
            try:
                ...
                flash('Error updating event', category='error')
...
# Delete Event Function - seems to be working but may need to check database to be sure
